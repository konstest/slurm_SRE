...
from aiohttp import web, ClientSession, ClientTimeout, client_exceptions
...
async def healthcheck(request):
    ...
    try:
        auth_response = await request.app['client_session'] \
                                    .get("{}/health".format(getenv('AUTH_SERVICE_URL')),
        ...
        if 200 <= status < 300:
            ...
            return web.json_response({'errors': 'fail'}, status = 504)
    except Exception as e:
        request.app['logger'].exception("failed to connect to AUTH_SERVICE_URL {}".format(e))
        return web.json_response({'result': '{}'.format(e)}, status=502)


@routes.get('/healthStartup')
@track_to_prometheus
async def healthcheckStartup(request):
    """
    Хелзчек, который всегда 200
    :param request: объект с параметрами входящего запроса
    :return: 200 OK
...
async def on_startup(app) -> None:
    ...
    app['requests_middle_total'] = Counter('http_middle_requests_total',
                                    'The total number of HTTP middle requests handled by the application')
    app['request_middle_duration_seconds'] = Histogram('http_middle_server_request_duration_seconds',
                                                'The HTTP middle response duration',
                                                buckets=[0.005, 0.01, 0.025, 0.05, 0.1, 0.3, 0.5, 0.7, 0.9,
                                                         1, 1.1, 1.3, 1.5, 1.7, 1.9,
                                                         2.0, 2.5,
                                                         5,
                                                         10])
    prometheus_registry.register(app['requests_middle_total'])
    prometheus_registry.register(app['request_middle_duration_seconds'])
