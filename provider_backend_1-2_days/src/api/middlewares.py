...
def track_to_middle_prometheus(func):
    """
    Отправка метрик в Prometheus
    :return: объект ответа
    """
    ...
    async def wrapped(request):
        ...
        try:
            ...
            city = getenv('PROVIDER_CITY', 'unknown')
            prometheus_labels = {'app': 'provider_backend', 'city': city}
            app['request_middle_duration_seconds'].observe(prometheus_labels, end.total_seconds())
            app['requests_middle_total'].inc({**prometheus_labels})
            return response
    return wrapped
...
async def get_auth_header(request: web.Request):
    ...
    try:
        deadline = request.headers.getone('X-Slurm-RPC-Deadline')
        AUTH_IGNORE_TIMEOUT = int(getenv('AUTH_IGNORE_TIMEOUT', '0')) == 1
        if deadline and AUTH_IGNORE_TIMEOUT:
            timeout = float(deadline) - datetime.now().timestamp()
        if timeout <= 0:
            return web.json_response({'errors': [{'title': f"Auth service deadline timeout"}]}, status=504)
    except:
        # no header
        pass

    retries = float(getenv('AUTH_REQUEST_RETRIES', '1'))
...
#            continue
            ...
            app['logger'].error('error response: %s' % auth_response, exc_info=True)
            status = 504
            continue
